...
namespace HospiEnCasa.App.Persistencia.AppRepositorios
{
    public class RepositorioPaciente : IRepositorioPaciente
    {
        ...
        public PacientePer ObtenerHistoriasClinicas(int IdPaciente)
        {
            var asignado =
                from p1 in _context.Historia
                from p2 in _context.Pacientes
                from p3 in _context.Personas
                where p1.IdPaciente == IdPaciente
                where p2.IdPac == p2.IdPaciente
                where p3.IdPersona == p3.IdPersona
                select new PacientePer()
                {
                    IdPaciente = p1.IdMedico,
                    IdPersona = p1.IdPersona,
                    Id = p3.Id,
                    Nombres = p3.Nombres,
                    Apellidos = p3.Apellidos,
